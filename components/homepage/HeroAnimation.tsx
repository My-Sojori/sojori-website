...
export function HeroAnimation() {
  ...
  // Phase 1 (0-0.12): booking incoming. Phase 2 (0.12-1): orchestration timeline.
  const phase = t < 0.12 ? 'booking' : 'timeline';
  const timelineProg = phase === 'timeline' ? (t - 0.12) / 0.88 : 0;

  // Events: lane 0 Guest, 1 Sojori, 2 Staff, 3 Admin
  const events = [
    ...
    { t: 0.2, lane: 1, icon: '🔔', title: 'Push admin & client', tag: 'Notif', tone: 'info' as const },
    ...
    { t: 0.4, lane: 2, icon: '🧹', title: 'Ménage J+2', tag: 'Yasmine', tone: 'success' as const },
    ...
    { t: 0.8, lane: 2, icon: '🧹', title: 'Ménage final', tag: 'Karim', tone: 'success' as const },
  ...
  const toneStyles = {
    info: { bg: 'rgba(6,182,212,0.10)', border: 'rgba(6,182,212,0.4)', text: '#67e8f9' },
    primary: { bg: 'rgba(230,176,34,0.12)', border: 'rgba(230,176,34,0.45)', text: '#f4cf5e' },
    success: { bg: 'rgba(16,185,129,0.10)', border: 'rgba(16,185,129,0.4)', text: '#34d399' },
    warn: { bg: 'rgba(239,68,68,0.10)', border: 'rgba(239,68,68,0.4)', text: '#fca5a5' },
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 24,
        background: 'linear-gradient(180deg, rgba(20,20,28,0.7) 0%, rgba(10,10,15,0.5) 100%)',
        overflow: 'hidden',
        minHeight: 540,
      }}
    >
      ...
      <div
        style={{
          ...
          height: 280,
          borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(230,176,34,0.18), transparent 70%)',
          pointerEvents: 'none',
        }}
      />
      <div
      ...
      >
        <div
          style={{
            ...
            border: '1px solid rgba(230,176,34,0.3)',
          }}
        >
          <SojoriMark size={22} />
          <div style={{ lineHeight: 1.1 }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.4 }}>SOJORI · CORE</div>
      ...
      {/* Phase 1: Booking incoming card (0-12%) */}
      {phase === 'booking' && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 380, position: 'relative', zIndex: 2 }}>
          <div style={{ textAlign: 'center', animation: 'fade-up 0.6s ease' }}>
            ...
            <div className="glass" style={{ padding: '24px 32px', maxWidth: 460, margin: '0 auto', borderRadius: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontWeight: 600, fontSize: 15 }}>Sarah Johnson</div>
            ...
            <div style={{ marginTop: 24, display: 'inline-flex', alignItems: 'center', gap: 8, color: 'var(--primary)', fontSize: 13, fontWeight: 500 }}>
          ...
          {lanes.map((lane) => (
            <div key={lane.id} style={{ position: 'relative', minHeight: 78, display: 'flex', alignItems: 'center', padding: '4px 0' }}>
              <div style={{ width: 100, flexShrink: 0, paddingRight: 12 }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: 1.5, color: lane.color, textShadow: `0 0 12px ${lane.color}66` }}>
                  {lane.label}
                </div>
                <div style={{ fontSize: 10, color: 'var(--text-3)' }}>{lane.sub}</div>
                <div style={{ width: 24, height: 2, marginTop: 5, borderRadius: 1, background: lane.color, boxShadow: `0 0 8px ${lane.color}` }} />
              </div>
              <div
                style={{
                  position: 'absolute',
                  ...
                  top: '50%',
                  height: 1,
                  background: `linear-gradient(90deg, transparent, ${lane.color}40 8%, ${lane.color}40 92%, transparent)`,
                  opacity: 0.7,
                }}
              />
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          gap: 6,
                          whiteSpace: 'nowrap',
                          boxShadow: age < 0.05 ? `0 0 0 4px ${tone.border}, 0 6px 16px rgba(0,0,0,0.4)` : `0 4px 12px rgba(0,0,0,0.3)`,
                          transition: 'box-shadow 0.4s ease',
                          zIndex: age < 0.05 ? 3 : 1,
                        }}
                      >
                        <span style={{ fontSize: 13 }}>{e.icon}</span>
                        <span style={{ fontSize: 10.5, fontWeight: 600, color: '#fff' }}>{e.title}</span>
                        <span
                          className="mono"
                          style={{
                            fontSize: 8.5,
          ...
          >
            <div
              style={{
                ...
                boxShadow: '0 0 16px rgba(230,176,34,0.6)',
                borderRadius: 3,
              }}
            />
          </div>
