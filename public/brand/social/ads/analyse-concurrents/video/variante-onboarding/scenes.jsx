...
/* couleurs plateformes (dispo dès le Hook) */
const BOOKING_C = '#0071C2';
...
function Hook(){
  ...
  // compteur de biens qui monte : 10 → 120 (parle à toutes les tailles de conciergerie)
  const bienCount = Math.round(interpolate([0.3,2.0],[10,120],easeO)(t));
  return (
    <Frame index={index} count={count} label="Conciergeries">
      <div style={{position:'absolute',left:72,right:72,top:380,...v}}>
        {/* ACCROCHE CIBLE — Airbnb dominant + l'hésitation PMS */}
        <div style={{fontFamily:C.sans,fontWeight:900,fontSize:104,lineHeight:.98,letterSpacing:'-.04em',color:C.ink}}>
          Vous êtes sur<br/><span style={{color:'#FF5A5F'}}>Airbnb</span> et vous<br/>hésitez pour un <span style={{background:C.grad,WebkitBackgroundClip:'text',backgroundClip:'text',color:'transparent'}}>PMS</span>&nbsp;?
        </div>
        {/* sous-accroche : le volume géré */}
        <div style={{marginTop:40,fontFamily:C.sans,fontWeight:700,fontSize:50,lineHeight:1.2,letterSpacing:'-.02em',color:C.ink2}}>
          <span style={{fontWeight:900,color:C.goldDeep}}>{bienCount} biens</span> à gérer&nbsp;?
        </div>
        <div style={{marginTop:22,fontFamily:C.sans,fontSize:36,lineHeight:1.4,color:C.ink3,fontWeight:400}}>
          Sans tout reconfigurer à la main.
        </div>
        {/* logos plateformes : ce que la conciergerie gère déjà */}
        <div style={{marginTop:70,display:'flex',alignItems:'center',gap:18,flexWrap:'wrap'}}>
          {['Airbnb','Booking','Vrbo','+40 OTA'].map((p,i)=>{
            const pv = interpolate([1.0+i*0.22,1.0+i*0.22+0.3],[0,1],easeO)(t);
            const col = p==='Airbnb' ? '#FF5A5F' : p==='Booking' ? BOOKING_C : C.goldDeep;
            return (
              <div key={p} style={{opacity:pv,transform:`scale(${0.8+pv*0.2})`,
                fontFamily:C.sans,fontWeight:800,fontSize:34,color:col,
                background:'rgba(255,255,255,.8)',border:`2px solid ${col}33`,
                borderRadius:999,padding:'16px 28px'}}>{p}</div>
            );
          })}
        </div>
      </div>
...
function Duel(){
  ...
  return (
    <Frame index={index} count={count} label="Face-à-face">
      <div style={{position:'absolute',left:72,right:72,top:470,...v}}>
        <div style={{fontFamily:C.mono,fontSize:30,fontWeight:600,letterSpacing:'.18em',textTransform:'uppercase',color:C.goldDeep,textAlign:'center',marginBottom:28}}>Face-à-face sur <span style={{color:AIRBNB}}>Airbnb</span></div>
...
function Solution(){
  ...
  const v = vis(t,dur);
  return (
    <Frame index={index} count={count} label="Ce que fait Sojori">
      <div style={{position:'absolute',left:72,right:72,top:480,...v}}>
        <div style={{fontFamily:C.mono,fontSize:30,fontWeight:600,letterSpacing:'.18em',textTransform:'uppercase',color:C.goldDeep,textAlign:'center',marginBottom:14}}>Avec <span style={{color:C.ink}}>Sojori</span></div>
  ...
  return (
    <Frame index={index} count={count} label="Gratuit · instantané">
      <div style={{position:'absolute',left:72,right:72,top:0,bottom:0,display:'flex',flexDirection:'column',
        ...
        <div style={{marginTop:54,fontFamily:C.sans,fontWeight:800,fontSize:76,lineHeight:1.08,letterSpacing:'-.035em',color:C.ink,maxWidth:860}}>
          Le PMS des<br/><span style={{background:C.grad,WebkitBackgroundClip:'text',backgroundClip:'text',color:'transparent'}}>conciergeries pro</span><br/>au Maroc
        </div>
        ...
        <div style={{marginTop:34,fontFamily:C.mono,fontSize:30,fontWeight:600,letterSpacing:'.08em',color:C.ink2,textTransform:'uppercase',
          ...
          <span style={{width:9,height:9,borderRadius:999,background:C.gold}}/>Migration automatique
        </div>
      </div>
    </Frame>
  );
...
/* =================== SCENE — CASCADE ONBOARDING =================== */
const BOOKING = BOOKING_C;
const CASCADE_ITEMS = [
  { text:<>Vos biens <span style={{color:'#FF5A5F',fontWeight:800}}>Airbnb</span> importés</> },
  { text:<>Calendriers &amp; réservations synchronisés</> },
  { text:<>Diffusion <span style={{color:BOOKING,fontWeight:800}}>Booking</span>, Vrbo <span style={{color:C.goldDeep,fontWeight:800}}>+40 OTA</span></> },
  { text:<>Votre site web pro en ligne</> },
  { text:<>Dynamic Pricing activé</> },
];
function Cascade(){
  const _sc = useScene(); let t = Number.isFinite(_sc.localTime)?_sc.localTime:0; const { dur, index, count } = _sc;
  const v = vis(t,dur);
  return (
    <Frame index={index} count={count} label="Configuration auto">
      <div style={{position:'absolute',left:72,right:72,top:430,...v}}>
        <div style={{fontFamily:C.mono,fontSize:30,fontWeight:600,letterSpacing:'.18em',textTransform:'uppercase',color:C.goldDeep,textAlign:'center',marginBottom:14}}>Automatiquement</div>
        <div style={{fontFamily:C.sans,fontWeight:800,fontSize:62,letterSpacing:'-.03em',color:C.ink,textAlign:'center',marginBottom:48}}>
          <span style={{color:C.goldDeep}}>Sojori</span> s&apos;occupe de tout
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:20}}>
          {CASCADE_ITEMS.map((it,i)=>{
            const start = 0.35 + i*0.72;
            const rv = interpolate([start,start+0.35],[0,1],easeO)(t);
            const pop = interpolate([start+0.1,start+0.45],[0,1],easeO)(t);
            return (
              <div key={i} style={{...glassCard({padding:'26px 30px'}),display:'flex',alignItems:'center',gap:24,
                opacity:rv,transform:`translateX(${(1-rv)*-34}px)`}}>
                <div style={{width:58,height:58,borderRadius:999,flexShrink:0,display:'grid',placeItems:'center',
                  background:C.grad,boxShadow:'0 10px 24px rgba(230,176,34,.35)',
                  transform:`scale(${0.4+pop*0.6})`}}>
                  <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#2A1E08" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12.5l5 5L20 6"/></svg>
                </div>
                <div style={{fontFamily:C.sans,fontWeight:700,fontSize:40,color:C.ink,letterSpacing:'-.01em'}}>{it.text}</div>
              </div>
            );
          })}
        </div>
...
/* =================== SCENE — SLOGAN ONBOARDING =================== */
function SloganOnb(){
  const _sc = useScene(); let t = Number.isFinite(_sc.localTime)?_sc.localTime:0; const { dur, index, count } = _sc;
  const v = vis(t,dur);
  const subV = interpolate([0.9,1.4],[0,1],easeO)(t);
  return (
    <Frame index={index} count={count} label="Avec Sojori">
      <div style={{position:'absolute',left:72,right:72,top:0,bottom:0,display:'flex',flexDirection:'column',
        alignItems:'center',justifyContent:'center',textAlign:'center',...v}}>
        <div style={{fontFamily:C.mono,fontSize:30,fontWeight:600,letterSpacing:'.18em',textTransform:'uppercase',color:C.goldDeep,marginBottom:30}}>Avec Sojori</div>
        <div style={{fontFamily:C.sans,fontWeight:900,fontSize:96,lineHeight:1.0,letterSpacing:'-.04em',color:C.ink}}>
          Passer au <span style={{background:C.grad,WebkitBackgroundClip:'text',backgroundClip:'text',color:'transparent'}}>PMS</span><br/>n&apos;a jamais été<br/>aussi <span style={{background:C.grad,WebkitBackgroundClip:'text',backgroundClip:'text',color:'transparent'}}>simple.</span>
        </div>
        <div style={{marginTop:44,opacity:subV,transform:`translateY(${(1-subV)*18}px)`,
          fontFamily:C.sans,fontSize:42,color:C.ink2,fontWeight:600,lineHeight:1.35}}>
          Vos biens <span style={{fontWeight:800,color:'#FF5A5F'}}>Airbnb</span> pilotés<br/>comme un <span style={{fontWeight:900,color:C.goldDeep}}>hôtel 5★</span>.
        </div>
      </div>
    </Frame>
  );
...
window.SOJORI_SCENES = { Hook, Revenu, Calendrier, Concurrents, Duel, Bilan, Solution, Cascade, SloganOnb, CTA };
